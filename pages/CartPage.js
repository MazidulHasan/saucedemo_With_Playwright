...
export default class CartPage extends BasePage {
    ...
    async verifyCartPageVisible() {
        try {
            ...
            await this.waitForElement(this.locators.cartList, 10000);

            // Check if title is visible
            const titleVisible = await this.isVisible(this.locators.pageTitle);

            if (titleVisible) {
                this.logger.pass('Cart page is visible and loaded');
                return true;
            } else {
                this.logger.fail('Cart page title not visible');
                return false;
            }

        } catch (error) {
            this.logger.fail(`Failed to verify cart page: ${error.message}`);
            await this.takeScreenshot(`cart-page-verification-error-${Date.now()}`);
            throw error;
        }
    }
    ...
    async isOnCartPage() {
        ...
        const onCartPage = url.includes('cart.html');

        if (onCartPage) {
            this.logger.pass('User is on cart page');
        } else {
            this.logger.warn('User is NOT on cart page');
        }
